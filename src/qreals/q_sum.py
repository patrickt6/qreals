r"""Alex's continued-fraction algorithm for the q-real sum [x]_q + [y]_q.

The algorithm, presented at the 2026-05-25 supervisor meeting, runs the
...
The not-always-reduced caveat. The meeting recorded that the algorithm's
...
the-limits step the meeting agreed is independent of the algorithm).
...
relies on. Its meeting-form building block is T_q(a) = [[ [a]_q, q^a ],
[1, 0]] at 1-indexed-odd positions, [[ [a]_{1/q}, q^{-a} ], [1, 0]] at even
positions; the first column is (R_x, S_x) with [x]_q = R_x / S_x. The 4x4
transfer matrix Ivan introduced is the Kronecker product T_q(a) (x) T_q(b),
...
def transfer_matrix(cf: list[int]) -> sp.Matrix:
    ...
    Returns:
        ...
        operator picture from the 2026-05-25 meeting.
    """
...
# ----------------------------------------------------------------------------
# Alex's CF algorithm on rational inputs
# ----------------------------------------------------------------------------
@dataclass(frozen=True)
class QSumRational:
    """Result of the Alex CF algorithm for [x]_q + [y]_q on rational x, y.
...
def q_sum_rational(x, y) -> QSumRational:
    """Alex's CF algorithm for [x]_q + [y]_q on rational x, y > 0.
    ...
    return QSumRational(
        ...
        caveat=(
            "Alex's CF algorithm does not always return a reduced fraction; "
...
def q_sum_irrational(x, y, N: int) -> QSumIrrational:
    ...
    Runs Alex's CF algorithm on the below-convergents x_n, y_n of x and y at
...
def finiteness_check(x, y, N: int) -> FinitenessReport:
    ...
    power of q in the denominator is exactly the algorithm's output shape from
    the meeting summary). For irrational x, y the function runs the convergent
    ...
    catalogue (Ovsienko Example 6.4; the worked 4 +/- sqrt(7) example of the
    2026-05-25 meeting).
