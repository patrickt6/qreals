...
# The largest integer part of x for which the q-negation is computed. The
# Jouteur negation [-x]_q has denominator (q - 1)[x]_q + 1, whose valuation is
# floor(x); the working precision must exceed floor(x) or the truncated
# denominator collapses to the zero series and cannot be inverted. Past this
# many integer units the series is prohibitively expensive (it lives at scale
# q^-floor(x)), so we stop with a clear error rather than block for seconds.
_MAX_NEG_INT_PART = 512


def _negation_prec(x_repr: str, N: int) -> int:
    """Working precision for [-x]_q: enough to clear floor(x), else a clear error.

    [-x]_q is a Laurent series of valuation about -(floor(x) + 1), so the input
    series [x]_q must be carried past q^floor(x). Raises ValueError when the
    integer part of x is too large to compute in reasonable time.
    """
    import sympy as sp

    m = int(sp.floor(sp.Abs(parse_real(x_repr))))
    if m > _MAX_NEG_INT_PART:
        raise ValueError(
            f"x has a large integer part (|floor(x)| = {m}); [-x]_q is a Laurent "
            f"series at scale q^-{m + 1} and would need that many coefficients to "
            f"compute. The x -> -x symmetry is studied for x near the origin; "
            f"try |x| <= {_MAX_NEG_INT_PART}."
        )
    return m + N + _BUFFER
